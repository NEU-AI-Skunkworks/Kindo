...
from kindo import globals, abs_path
...
def _train_model(
    ...
    if isinstance(model.env, DummyVecEnv):
        ...
        model_dir = f"{globals.save_path}/{model.env.env.__class__.__name__}/{model_name}"
    ...
    model.env.close()
    logger.info(
        f"Model {model.__class__.__name__} is trained and saved to {model_dir}"
    )
...
def train_a_model(
    ...
    _train_model(
        model, total_timesteps, log_interval, model_name, save_history, maximum_episode_reward
    )
