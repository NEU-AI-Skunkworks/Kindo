...
from kindo.baselines_train import train_a_couple_of_models, train_a_model  # noqa
...
def get_saved_environments() -> typing.List[str]:
    return [
        env_path.replace("saved/", "")
        for env_path in glob.glob(f"{globals.save_path}/*")
    ]


def get_trained_model_paths(env_name):
    return [model_path for model_path in glob.glob(f"{globals.save_path}/{env_name}/*")]
